import { Router } from 'express';
import * as ctrl from '../controllers/skillController.js';
import { skillValidator } from '../validators/index.js';
import { validate } from '../middleware/validate.js';
import { auth, authorize } from '../middleware/auth.js';

const router = Router();

// Public
router.get('/', ctrl.getAll);

// Admin
router.get('/admin', auth, authorize('superadmin', 'admin'), ctrl.adminGetAll);
router.post('/', auth, authorize('superadmin', 'admin', 'editor'), skillValidator, validate, ctrl.create);
router.put('/:id', auth, authorize('superadmin', 'admin', 'editor'), skillValidator, validate, ctrl.update);
router.delete('/:id', auth, authorize('superadmin', 'admin'), ctrl.remove);

export default router;
